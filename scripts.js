...
for(let i = 0; i < boxes.length; i++) { //Quando alguem clica na caixa
    boxes[i].addEventListener("click", function() {
        let el = checkEl(player1,player2);
        //verifando se ja tem valor no quadrado
...
function computerPlay() {
    ...
    for(let i = 0; i <boxes.length; i++) {
        ...
        //so vai preencher se tiver filho vazio
        if(boxes[i].childNodes[0] == undefined) { 
            if(randomNumber <= 1) {
        ...
        //checagem de qtos foram preenchidos
        } else {
            filled++;
        }
    }
